...
from typing import Optional, Callable, List, Tuple, Dict, TypeVar, Generic, Coroutine, Any
...
class FuzzyChoicesCache(Generic[T]):

    def __init__(self, fetch_method: Callable[[None], Coroutine[None, None, T]], threshold: int = 7):
...
class DependentFuzzyChoicesCache:
    ...
    def __init__(self, fetch_method: Callable[[Any], Coroutine[None, Any, AutoCompleteItem]], sanitizer: Optional[Callable[[str], str]] = None, threshold: int = 7):
        self.fetch_method = fetch_method
        self.threshold = threshold
