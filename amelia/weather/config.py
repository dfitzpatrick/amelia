...
class MetarConfigGroup(app_commands.Group):
    ...
    @app_commands.command(name='add-channel', description='Adds a channel that will allow the output of metar')
    ...
    async def add_metar_channel(self, itx: Interaction, channel: discord.TextChannel):
        ...
        if channel not in self.cache.channels.item.get(channel.guild.id, []):
            ...
            response = f"{channel.name} added to approved metar channels"
        else:
            response = f"{channel.name} is already added."
        await itx.response.send_message(response, ephemeral=True)

    @app_commands.command(name='remove-channel', description="Removes a channel from displaying metar output")
    ...
    async def remove_metar_channel(self, itx: Interaction, channel: discord.TextChannel):
        ...
        await itx.response.send_message(f"{channel.name} is cleared from allowed metar channels", ephemeral=True)

    @app_commands.command(name='list-channels', description="Lists the current channels that allow metar output")
    @app_commands.checks.has_permissions(manage_channels=True)
